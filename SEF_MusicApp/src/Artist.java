...
public class Artist 
{
	...
	private boolean preConditionsToAddArtist()
	{
		...
		boolean isValidGeners = false;
			...
			if(this.Genres.size() >= 2 && this.Genres.size() <= 5 &&!(this.Genres.contains("rock") && this.Genres.contains("pop")))
			{
				isValidGeners = true;
			}
			
			return isValidArtistId && isValidDate &&isValidAddress && isValidBio && isValidOccupation && isValidAwards && isValidGeners;
	}
